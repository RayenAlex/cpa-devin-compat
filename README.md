...
| Responses 请求带 `namespace` 工具（常见于 MCP 工具）时上游返回 502 `invalid_argument` | CPA 把 namespace 转成工具分组，Devin 执行器会发出一个无名工具 | 把 namespace 里的子工具展开成顶层 function 工具（保留原名，与已有工具重名的跳过） |
...
plugins:
  ...
  configs:
    cpa-devin-compat:
      ...
      flatten-namespace-tools: true    # 展开 Responses 的 namespace 工具
